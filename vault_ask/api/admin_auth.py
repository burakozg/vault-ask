...
Single shared key in the ``X-API-Key`` header, compared in constant time. The
key is read from ``app.state`` (set by ``create_app``'s lifespan) rather than
the process environment directly, so tests can build an app with a key of
their own choosing without touching os.environ.

Repeated failures from one address are then throttled. Constant-time
comparison defeats timing attacks and says nothing about volume, and this
service listens on the LAN with no other rate limit in front of it — one
compromised device is enough to sit and guess. The counter is deliberately in
memory: an unauthenticated request must not be able to make the service write
to disk, which is a denial-of-service in its own right.

Mirrors podcast-digest's ``api/auth.py`` — same constants, same shape —
because this is the one piece of the admin portal with no reason to diverge
between sibling apps.
"""
...
async def require_api_key(
    ...
    if not expected:
        ...
        raise HTTPException(
            ...
            detail="admin API key is not configured; set VAULTASK_ADMIN_API_KEY",
        )
